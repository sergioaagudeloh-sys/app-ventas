...
export default function OrderTracking() {
  ...
  const { config, fetchConfig, isLoading: configLoading } = useAppConfigStore()
  ...
  useEffect(() => {
    ...
    if (!configLoading) {
      if (config && config.orderTrackingEnabled === false) {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-primary)] px-4 py-12">
        <div className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-8 shadow-xl text-center backdrop-blur-md">
          ...
          <div className="space-y-3">
            <Link 
              to="/" 
              className="w-full inline-flex items-center justify-center px-5 py-3 rounded-xl bg-[var(--accent-color)] hover:bg-[var(--accent-hover)] text-white font-medium transition-all duration-300 shadow-lg shadow-[var(--accent-color)]/20"
            >
            ...
            {config?.whatsappContact && (
              <a 
                ...
                className="w-full inline-flex items-center justify-center px-5 py-3 rounded-xl border border-[var(--border-color)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] font-medium transition-all duration-300 gap-2 text-sm"
              >
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] px-4 py-8 md:py-16 transition-colors duration-300">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2 mb-4">
          <div className="w-12 h-12 bg-gradient-to-tr from-[var(--accent-color)] to-indigo-500 rounded-2xl flex items-center justify-center shadow-lg transform rotate-6">
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden backdrop-blur-md">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--accent-color)]/5 rounded-full blur-3xl -z-10" />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--border-color)] pb-6 mb-6">
            <div>
              ...
              <h2 className="text-lg md:text-xl font-bold font-mono text-[var(--accent-color)] mt-0.5">
            ...
            <div className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold ${statusInfo?.color} transition-all duration-300`}>
          ...
          {statusInfo?.stepIndex !== -1 ? (
            <div className="py-6 border-b border-[var(--border-color)] mb-6">
              <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-8">
                Progreso de tu Pedido
              </h3>
              
              <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-8 md:gap-4">
                {/* Línea de fondo en pantallas grandes */}
                <div className="absolute left-4 top-8 bottom-8 md:left-0 md:top-6 md:bottom-auto md:w-full md:h-1 bg-[var(--border-color)] -z-10" />
                
                {/* Línea de progreso activa */}
                <div 
                  className="absolute left-4 top-8 bottom-8 md:left-0 md:top-6 md:bottom-auto md:h-1 bg-gradient-to-r from-[var(--accent-color)] to-indigo-500 transition-all duration-500 -z-10"
                  style={{
                    width: window.innerWidth >= 768 ? `${(statusInfo.stepIndex / (steps.length - 1)) * 100}%` : '4px',
                    height: window.innerWidth < 768 ? `${(statusInfo.stepIndex / (steps.length - 1)) * 100}%` : '4px',
                  }}
                />
                  ...
                  return (
                    <div key={idx} className="flex md:flex-col items-center md:items-center gap-4 md:gap-2 w-full md:text-center relative">
                      ...
                      <div className="flex flex-col md:items-center">
                        <span className={`text-sm font-bold ${isCompleted ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'} ${isActive ? 'text-[var(--accent-color)]' : ''}`}>
                          {step.label}
                        </span>
            ...
            <div className="py-6 border-b border-[var(--border-color)] mb-6 text-center">
              <div className="inline-flex items-center gap-2 text-rose-500 bg-rose-500/10 px-4 py-3 rounded-xl border border-rose-500/20 text-sm font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)]/40 p-4 rounded-xl space-y-3">
              ...
              <div className="text-sm">
                ...
                <p className="text-[var(--text-secondary)] text-xs mt-0.5">
                  Método de Entrega: <span className="font-medium text-[var(--text-primary)]">{order.metodoEntrega === 'DOMICILIO' ? 'Envío a Domicilio' : 'Retiro en Tienda'}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)]/40 p-4 rounded-xl space-y-3">
              ...
              <div className="text-sm space-y-1">
                <div className="flex justify-between text-xs text-[var(--text-secondary)]">
                  ...
                  <span className="font-medium text-[var(--text-primary)]">{order.metodoPago || 'N/A'}</span>
                </div>
                <div className="flex justify-between font-bold text-sm text-[var(--text-primary)] pt-1 border-t border-[var(--border-color)]/30 mt-2">
                  <span>Total Pagado/Por Pagar:</span>
                  <span className="text-[var(--accent-color)]">{formatCurrency(order.total || 0)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-3 flex items-center gap-1.5">
              <ShoppingBag className="w-3.5 h-3.5" />
              Detalle de Productos
            </h3>
            <div className="divide-y divide-[var(--border-color)] border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-secondary)]/30">
              {order.productos?.map((prod, idx) => (
                <div key={idx} className="flex justify-between items-center p-3.5 text-sm hover:bg-[var(--bg-secondary)]/50 transition-colors duration-200">
                  <div className="flex flex-col">
                    <span className="font-semibold text-[var(--text-primary)]">{prod.nombre}</span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[var(--text-secondary)] px-2">
          ...
          <Link 
            to="/" 
            className="flex items-center gap-1 hover:text-[var(--accent-color)] transition-colors duration-200"
          >
            Volver a la tienda
